...
class ClusteringResultsWriter:
    ...
    def write(results, filepath):
        ClusteringResultsWriter._logger.info("Writing results to " + str(filepath))
        dirname = os.path.dirname(filepath)
        basename = os.path.basename(filepath)
        filename = os.path.splitext(basename)[0]
        results.cells.to_csv(os.path.join(dirname, filename + '.csv'))
        metadata = {
            'config': results.config.to_dict(),
            'samples': results.sample_sets
        }
        with open(os.path.join(dirname, filename + '.yml'), 'w') as metafile:
